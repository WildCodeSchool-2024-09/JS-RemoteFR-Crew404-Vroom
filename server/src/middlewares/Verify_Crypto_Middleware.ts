...
async function Verify_Crypto_Middleware(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (!Array.isArray(dataToken) || dataToken.length === 0) {
      ...
      console.error({
        ...
        explication3:
          "On n'a pas besoin de vérifier la date d'expiration si le token n'existe pas.",
      });
      return;
    }
    ...
    if (new Date(dataToken[0].expires_at) < new Date()) {
      ...
      const [deleteToken] = await usePoolConnection.query<RowDataPacket[]>(
        "DELETE FROM reset_password WHERE token = ?",
        [req.body.token],
      );
      ...
      if (deleteToken.length === 0) {
        res.status(500).json({
          error:
            "Erreur interne serveur + Token expiré. Veuillez refaire la demande.",
        });
        console.error({
          identity: "Verify_Crypto_Middleware.ts",
          type: "middleware",
          ...
          explication1:
            "La date d'expiration du token est inférieure à la date actuelle.",
          explication2: "Le token n'est plus valide.",
          explication3:
            "Le server a essayé de supprimer le token de la DB et cela a échoué.",
