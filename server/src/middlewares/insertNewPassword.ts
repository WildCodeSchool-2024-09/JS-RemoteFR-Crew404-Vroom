...
async function InsertNewPassword(
  ...
  try {
    ...
    const [results] = await usePoolConnection.query<ResultSetHeader>(
      ...
      [req.body.password, req.body.dataUser.id],
    );
    ...
    if (results.affectedRows === 0) {
      ...
      console.error({
        ...
        cause1:
          "l'id utilisateur n'a pas été trouvé dans dataUser qui est mis à disposition par Verify_Crypto_Middleware.ts",
        cause2:
          "L'Erreur viens forcément de Verify_Crypto_Middleware.ts qui gère le token",
      });
      return;
    }
